 ...
 * nuevamente a la funcion con el numero cantidad_cola-n para que la cola quede ordenada.
*/
...
/*
 *Copia la cola a una cola auxiliar para poder ser iterada (ya que si se itera sobre la cola original, se desordena).
*/
void copiar_cola(cola_t* cola, cola_t* cola_aux){
	int iteraciones=cola_cantidad(cola);
	for(int i=0;i<iteraciones;i++){
		void* elemento=cola_primero(cola);
		cola_encolar(cola_aux,elemento);
...
lista_iterador_t* lista_iterador_crear(lista_t* lista){
	...
	copiar_cola(lista->cola,cola_aux);
	iterador->cola=cola_aux;
